...
-- Проверка уникальности букв в слове (UTF-8)
local function is_unique(word)
    local seen = {}
    for uchar in word:gmatch("([%z\1-\127\194-\244][\128-\191]*)") do
...
local function count_duplicates(combination, frequent_letters, check_frequent)
    ...
    if check_frequent then
        ...
        for _, cnt in pairs(letter_count) do
...
-- Частые буквы русского языка (UTF-8 символы)
local frequent_letters = { ["о"] = true, ["е"] = true, ["а"] = true, ["и"] = true, ["н"] = true }

-- Чтение словаря и фильтрация
local words = {}
for line in io.lines(dict_file) do
    local w = line:lower():gsub("%s+", "")
    -- Устанавливаем targetLen по длине первого слова (символы UTF-8)
    if #words == 0 and not targetLen then
        targetLen = utf8.len(w)
    end
    if utf8.len(w) == targetLen and is_unique(w) then
